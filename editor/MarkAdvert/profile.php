...
$query = "select * from tbl_employee, tbl_department, tbl_employment_type, tbl_address WHERE email = '$username' ";
      ...
      $select_empl_by_id = mysqli_query($connection, $query);
                 ...
                 while ($row = mysqli_fetch_assoc($select_empl_by_id)) {
                    ...
                    $title = mysqli($row['title']);
                    $dept = mysqli($row['dept']);
                    $deptname = mysqli($row['deptname']);
                    $empttype = mysqli($row['empttype']);
                    $empname = mysqli($row['name']);                    
                    $maried_status = mysqli($row['maried_status']);
                    $fname = mysqli($row['firstname']);
                    $midlename = mysqli($row['midlename']);
                    $lastname = mysqli($row['lastname']);
                    $gender = mysqli($row['gender']);
                    $phone = mysqli($row['phone']);
                    $email = mysqli($row['email']);
                    $birthdate = mysqli($row['birthdate']);
                    $salary = mysqli($row['salary']);
                    $datejoin = mysqli($row['datejoin']);
                    $image = mysqli($row['image']);
                    $address = mysqli($row['address']);
                  }
      ...
      $select_adre = mysqli_query($connection, $adrquery);
                ...
                while ($row = mysqli_fetch_assoc($select_adre)) {
                  $ad_id = mysqli($row['adre_id']);
                  $zipcode = mysqli($row['zip_cod']);
                  $street = mysqli($row['street']);
                  $city = mysqli($row['city']);
                  $state = mysqli($row['state']);
                  $nation = mysqli($row['country']);
                }
       ...
       if (isset($_POST['update_employ'])) {
                    ...
                    $zipcode = mysqli($_POST['zipcode']);
                    $city = mysqli($_POST['city']);
                    $nation = mysqli($_POST['nation']);
                    $maried = mysqli($_POST['maried']);
                    $mname = mysqli($_POST['mname']);
                    $gender = mysqli($_POST['gender']);
                    $street = mysqli($_POST['street']);
                    $state = mysqli($_POST['state']);                    
