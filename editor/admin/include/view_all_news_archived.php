...
<input class="form-control" id="myInput" type="text" placeholder="Search..">
  ...
  $page = mysqli_real_escape_string($connection,$_GET['page']);
    ...
    $query_count = "SELECT * from tbl_news, tbl_image, tbl_news_category, tbl_employee where authorid = id and categoryID = newscat_id and img_id = image_id and '$cdate' > end_date order by date_published";
...
<table class="table table-bordered table-hover">
  ...
  $cdate = date("Y-m-d");
  //echo "$cdate";
  $query = "SELECT * from tbl_news, tbl_image, tbl_news_category, tbl_employee where authorid = id and categoryID = newscat_id and img_id = image_id and '$cdate' > end_date order by date_published LIMIT $page1,5";
  ...
  while ($row = mysqli_fetch_assoc($select_query)) {
    ...
    echo "<td>$endate</td>";
    echo "<td><a href='news.php?source=current_news&newsid=$newsid&did=$did'>TakeOff</a> | <a href='news.php?source=edit_compose&did=$did&usr=$username'>Publish Again | <a href='news.php?source=prev&did=$newsid'>View</a></td>";
    echo "</tr>";
  }
       ?>
